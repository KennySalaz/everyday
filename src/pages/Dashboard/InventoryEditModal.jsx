...
export default function InventoryEditModal({ isOpen, onClose, onSave, item }) {
    ...
    // Pre-cargar campos cuando se abre con un item
    useEffect(() => {
        if (isOpen && item) {
            setInvName(item.name || '');
            setInvType(item.type || 'materia');
            setInvQty(item.totalQty || 1);
            // Formatear el precio con comas
            const priceStr = item.totalPrice != null
                ? item.totalPrice.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g, ',')
                : '';
            setInvTotal(priceStr);
            setInvIcon(item.icon || 'Gem');
            setInvImage(null);
            setInvImagePreview(item.imageUrl || '');
            setInvSku(item.sku || '');
            setInvLink(item.link || '');
    ...
    const formatPrice = (raw) => {
        ...
        return intPart.replace(/\B(?=(\d{3})+(?!\d))/g, ',') + (decPart !== undefined ? '.' + decPart.slice(0, 2) : '');
    };
    ...
    const handleSubmit = async (e) => {
        ...
        let finalImageUrl = invImagePreview; // mantener la imagen existente por defecto
        ...
        const history = item.priceHistory ? [...item.priceHistory] : [];
        if (item.totalPrice !== total || item.totalQty !== qty) {
            history.push({
                ...
                <>
                    ...
                    >
                        ...
                        >
                            ...
                            <h2 style={{ fontSize: '1.5rem', color: 'var(--color-primary)', marginBottom: '1.5rem' }}>Editar Insumo</h2>
                            ...
                            <form onSubmit={handleSubmit}>
                                    ...
                                    <div className="input-group">
                                        ...
                                        <div style={{ position: 'relative' }}>
                                            ...
                                            <input
                                                ...
                                                className="styled-input"
                                                value={invTotal}
                                                onChange={e => setInvTotal(formatPrice(e.target.value))}
                                                placeholder="0.00"
                                ...
                                <div className="form-grid" style={{ marginBottom: '2rem' }}>
                                    ...
                                    <div className="input-group">
                                        ...
                                        <div style={{
                                            ...
                                            background: 'var(--color-background-off)',
                                            height: '140px'
                                        }} onClick={() => document.getElementById('inv-edit-image-upload').click()}>
                                            {invImagePreview ? (
                                                <>
                                                    <img
                                                        src={invImagePreview}
                                ...
                                >
                                    {isInvUploading ? 'Guardando...' : <><Save size={20} /> Guardar Cambios</>}
                                </button>
                            </form>
                        </motion.div>
                    </motion.div>
